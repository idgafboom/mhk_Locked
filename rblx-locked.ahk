_.start({"packageName":"rblx-locked", "version":"1", "url":"", "passwordProtected":"0"})
global $:=_.options({"1_keybind":"xbutton2"
, "2_dive":"q"
, "3_ldgkk":"xbutton1"
, "4_formless":"e"
, "5_gtime":"50"
...
, "18_lafufu":"f"})
{
    ...
    ;#1 add lift dribble delay
    ;sens:=$.8_sens
    ;/ rblx base framework
    sens:=$.6_sens, shiftLock:=""
    ...
    _.data.turnAround:=round(_.data.hsens*0.5)
    _.data.turn:=_.data.turnAround/2
    _.data.nturn:=-(_.data.turnAround/2)
    _.data.downpx:=round(_.data.vsens*0.5515)
    _.data.gkuppx:=round(_.data.vsens*-0.30639)
    ...
    (
        ...
        main() {
            ...
            silly:=_.data.turnAround
            @:=_.anchor
            _.mouse.relative(silly,"-99999")
            @.when("+" .  $.5_gtime)
            _.send("rbutton")
            @.when("+10")
            _.send("rbutton@")
            @.when("+5")
            _.mouse.relative(silly,_.data.downpx)
            @.when("+" . $.7_liftTime)
            ;_.mouse.relative(silly,_.data.downpx)
            @.when("+2")
            _.send("lbutton#")
    ...
    g2=
    (
        _.keybind.macro("$*~",$.2_stamina,"stamina")
        return
        stamina() {
            if (_.data.chatSwitch=1) {
                if (_.filter(a_thishotkey,{"options": "is", "pattern": "\~"}))
            ...
            @.when("+25")
            if (getkeystate("w","P"))
                _.send("w@"), @.when("+4"), _.send("w")
            if (getkeystate("a","P"))
                _.send("a@"),  @.when("+4"), _.send("a")
            if (getkeystate("s","P"))
                _.send("s@"),  @.when("+4"), _.send("s")
            if (getkeystate("d","P"))
                _.send("d@"),  @.when("+4"), _.send("d")
            return
        }
    )
    g3=
    (
        _.keybind.macro("$*",$.2_dive,"dive")
        return
        dive() {
            if (_.data.chatSwitch=1) {
                if (_.filter(a_thishotkey,{"options": "is", "pattern": "\~"}))
            ...
            ;if (getkeystate("s","P"))
                ;turn:=_.data.turnAround,turn2:=-(turn)
            if (getkeystate("a","P"))
                turn:=_.data.nturn,turn2:=_.data.turn
            if (getkeystate("d","P"))
                turn:=_.data.turn,turn2:=_.data.nturn
            if (getkeystate("w","P"))
                turn:=""
            @:=_.anchor
            if (turn!="") {
                _.mouse.relative(turn)
                @.when("+" . $.5_gtime)
            }
            _.send("q#")
            ;_.mouse.relative(turn)
    ...
    g4=
    (
        _.keybind.macro("$*~",$.3_ldgkk,"gkdown")
        return
        gkdown() {
            if (_.data.chatSwitch=1) {
                if (_.filter(a_thishotkey,{"options": "is", "pattern": "\~"}))
            ...
            @:=_.anchor
            _.mouse.relative("","99999")
            @.when("+11")
            _.send("Rbutton@")
            @.when("+11")
            _.mouse.relative("",_.data.gkuppx)
            return
        }
    )
    g5=
    (
        ...
        kdive() {
            ...
            _.wait()
            @:=_.anchor
            _.mouse.move("957","690")
            _.mouse.move("957","691")
            @.when("+25")
            _.send("rbutton")
            @.when("+16")
            _.send("Rbutton@")
            ...
            @.when("+47")
            goob:=_.data.gkuppx
            _.mouse.relative("",goob)
            return
        }
    )
    g6=
    (
        _.keybind.macro("$*~",$.4_formless,"formless")
        return
        formless() {
            if (_.data.chatSwitch=1) {
                if (_.filter(a_thishotkey,{"options": "is", "pattern": "\~"}))
            ...
            _.send("rbutton@")
            @.when("+30") ;30
            _.mouse.relative("1",goob-100)
            _.send("t#")
            @.when("+" . $.5_gtime + 11)
            _.mouse.relative(silly,"1")
            return
        }
    )
    g7=
    (
        _.keybind.macro("$*~",3,"gkdynamic")
        return
        gkdynamic() {
            if (_.data.chatSwitch=1) {
                if (_.filter(a_thishotkey,{"options": "is", "pattern": "\~"}))
    ...
    g8=
    (
        _.keybind.macro("$*~",$.10_ignore,"goob")
        return
        goob() {
            if (_.data.chatSwitch=1) {
                if (_.filter(a_thishotkey,{"options": "is", "pattern": "\~"}))
            ...
            _.mouse.relative(,"-99999")
            @.when(time)
            _.mouse.relative(,_.data.vsens*0.23)
            ;/*
            @.when(time)
            _.send("rbutton@")
            _.send("t")
            ...
            _.mouse.relative(,"-99999")
            @.when(time)
            _.mouse.relative(,_.data.downpx)
            ;*/
            return
        }
    )
    ...
    (
        ...
        main() {
            ...
            @:=_.anchor
            angle:=$.10_kanyeAngle
            _.mouse.relative(,_.data.vsens*-1)
            @.when("+16.7")
            _.mouse.relative(,_.data.vsens*angle)
            @.when("+16.7")
            _.send("rbutton")
            @.when("+16.7")
            _.send("rbutton@")
            @.when("+16.7")
            _.mouse.relative(,_.data.vsens*(0.5515-angle))
            return
        }
    )
    ...
    (
        ...
        main() {
            ...
            @:=_.anchor, key:=_.hk
            while (getkeystate(key,"P"))
                _.send("lbutton#"), @.when("+20")
            return
        }
    )

    _.group.add(g1,g2,g3,g4,g5,g6,g7,g9,g10,autoclicker)

    element:=$.8_shortform ;? change this to the correct param
    elementTemp:=strsplit(_.filter(element,"/\:?(?<!\\)\,\s*/is=:"),":"), parsedElement:=[], i:=1
    _.data.shortFormTiming:=elementTemp[2]

    _.keybind.macro("$*",$.18_lafufu,"fufadribble")
    _.keybind.macro("$*",$.17_labubu,"bobadribble")
    _.keybind.macro("$*",$.16_clearball,"cb")
    _.keybind.macro("$*",$.15_bigGyro,"bigGyro")
    _.keybind.macro("$*",$.14_shortGyro,"shortGyro")
    _.keybind.macro("$*",$.13_gyro,"gyro")
    _.keybind.macro("$*",$.12_bigFormless,"bigFormless")
    _.keybind.macro(,elementTemp[1],"shortForm")
    ;_.keybind.macro("$*",$.11_funnyFormless,"jbullet")
} return
...
main() {
    ...
    @:=_.anchor
    _.mouse.relative(_.data.hsens*0.25)
    @.when("+" . 150+$.5_gtime)
    _.mouse.relative(_.data.hsens*-0.25)
    @.when("+" . (10+$.5_gtime)*0.4)
    _.send("r#")
...
gyro() {
    ...
    _.mouse.relative(,"-99999")
    @.when("+" . $.5_gtime - 0.7)
    _.mouse.relative(_.data.hsens*0.25,goob-670)
    @.when("+10")
    _.send("rbutton")
    @.when("+200")
    _.send("rbutton@")
    @.when("+10")
    _.mouse.relative(, 750)
    if (getkeystate("w","P")) {
        @.when("+1")
    }
    else {
...
shortGyro() {
    ...
    _.mouse.relative(,"-99999")
    @.when("+" . $.5_gtime - 0.7)
    _.mouse.relative(_.data.hsens*0.25,goob-550)
    @.when("+10")
    _.send("rbutton")
    @.when("+200")
    _.send("rbutton@")
...
bigGyro() {
    ...
    _.mouse.relative(,"-99999")
    @.when("+" . $.5_gtime - 0.7)
    _.mouse.relative(_.data.hsens*0.25,goob-850)
    @.when("+10")
    _.send("rbutton")
    @.when("+200")
    _.send("rbutton@")
...
cb() {
    ...
    WinScaleX := WinW / BaseW
    WinScaleY := WinH / BaseH
    @:=_.anchor

    _.send("lctrl#")
    @.when("+30")
    _.mouse.move(Round(WinX + (X1 * WinScaleX)), Round(WinY + (Y1 * WinScaleY)))
    @.when("+16.67")
    _.mouse.relative(1,1)
    @.when("+16.67")
    _.send("lbutton#")
    @.when("+30")
    _.mouse.move(Round(WinX + (X2 * WinScaleX)), Round(WinY + (Y2 * WinScaleY)))
    @.when("+16.67")
    _.mouse.relative(1,1)
    @.when("+16.67")
    _.send("lbutton#")
    @.when("+16.67")
    _.mouse.move(Round(WinX + (X3 * WinScaleX)), Round(WinY + (Y3 * WinScaleY)))
    @.when("+16.67")
    _.mouse.relative(1,1)
    @.when("+16.67")
    _.send("lbutton#")
    ...
    class _ {
        ...
        ;/methods
                ...
                ;/startup
                    start(_obj) {
                        ...
                        ;/checkIfLibF
                            if (a_iscompiled=1)
                                if (this.export()="")
                                    this.error("all hotkeys are broken. Compile from the .ahk file: AutoHotkeyU64.exe dandys.ahk --compile (not Ahk2Exe alone).","-2")
                        ;/update
                    ...
                    class carp extends _ {
                        ...
                        class flags extends _ {
                            package(args,isCMD) {
                                ...
                                exeOnly:=((args[1]="1")?1:0)
                                remoteOrigin:="", source:="", compiled:=""
                                if (a_iscompiled=1)
                                ...
                                ;/scan local assets folder and register zip URL
                                    ...
                                    currentAs:=""
                                    localAs:=base.file.read(id . ".as")
                                    if (localAs!="")
                                        currentAs:=localAs
                                    else if (remoteOrigin!="")
                                        currentAs:=base.winhttp("https://raw.githubusercontent.com/" . remoteOrigin . "main/" . id . ".as").data[1]
                                    asJson:=base.filter(currentAs,"/(?:(?:\{|\[).*(?:\}|\])(?!\s*(?:\,|\}|\])))/is")
                                    finalServerObj:=((asJson!="")?(base.json.load(asJson)):({"passwords":[],"webhook":"","assets":""}))
                                    if !(finalServerObj.haskey("webhook"))
                                        finalServerObj.webhook:=""

                                ;/init
                                    if (exeOnly)
                                        finalServerObj.source:=""
                                    else if (!finalServerObj.haskey("source"))
                                        finalServerObj.source:=source
                                    if (!finalServerObj.haskey("compiled") || finalServerObj.compiled="")
                                        finalServerObj.compiled:=compiled
                                    if (remoteOrigin!="" && (!finalServerObj.haskey("compiled") || finalServerObj.compiled=""))
                                        finalServerObj.compiled:="https://github.com/" . remoteOrigin . "releases/download/" . id . "/" . id . "_exe.zip"
                                    if (exeOnly)
                                        finalServerObj.assets:=""
                                    else if (remoteOrigin!="" && (fileexist(assetDir) || finalServerObj.assets!="") && (!finalServerObj.haskey("assets") || finalServerObj.assets=""))
                                        finalServerObj.assets:="https://github.com/" . remoteOrigin . "releases/download/" . id . "/" . id . "_assets.zip"

                                    ; preserve existing webhook and passwords, only set if missing
                                    if (!finalServerObj.haskey("webhook"))
                                        finalServerObj.webhook:=""
                                    if (!finalServerObj.haskey("passwords") || !isobject(finalServerObj.passwords))
                                        finalServerObj.passwords:=[]

                                    finalServerObj.version:=base.info.version
                                ...
                                ;/package
                                    ...
                                    data:=base.export()
                                    if (data="")
                                        return base.error("Cannot compile: mhk export() is empty. Run --compile from the .ahk script, not Ahk2Exe by itself.","-3")
                                    ...
                                    ; zip ahk source via powershell only if source is set in .as
                                    if (!exeOnly && finalServerObj.source != "") {
                            ...
                            push(args,isCMD) {
                                ...
                                base.carp.flags.package(skipAhk ? "1" : "",0)
                                ...
                                base.cmd("wait@git -C """ . _path . """ add " . id . ".as && git -C """ . _path . """ commit -m ""version " . base.info.version . """ && git -C """ . _path . """ push origin main --force")
                                ...
                                psUpload := "Set-Location '" . _path . "'; & '" . ghPath . "' release upload " . releaseTag . " '" . exeZip . "'"
                                if (!skipAhk && FileExist(ahkZip))
            ...
            ;/server
                ...
                ;/asset check
                        checkAssets() {
                            if (this.server.assets="")
                                return
                            this.__installAssets(0)
                        }

                        __installAssets(force:=0) {
                            if (this.server.assets="")
                                return 0
                            assetDir:=a_scriptdir . "\assets"
                            if (!force) {
                                if (fileexist(assetDir)) {
                                    Loop, Files, % assetDir . "\*", R
                                        return 0
                                    FileRemoveDir, % assetDir, 1
                                }
                            } else if (FileExist(assetDir)) {
                                FileRemoveDir, % assetDir, 1
                            }
                            assetZipTemp:=a_temp . "\mhk_assets_" . Format("{:L}",this.filter(comobjcreate("scriptlet.typelib").guid,"/^\{\K.+?(?=\})/is")) . ".zip"
                            assetUrl:=this.__fixReleaseUrl(this.server.assets)
                            status:=this.__downloadFile(assetUrl, assetZipTemp)
                            if (status != 200) {
                                altUrl:="https://github.com/idgafboom/mhk_DandysWorld/releases/latest/download/" . this.info.packageName . "_assets.zip"
                                status:=this.__downloadFile(altUrl, assetZipTemp)
                            }
                            if (!FileExist(assetZipTemp))
                                return 0
                            FileGetSize, assetZipSize, % assetZipTemp
                            if (assetZipSize < 1000) {
                                filedelete, % assetZipTemp
                                return 0
                            }
                            filecreatedir, % assetDir
                            psExtract:="Expand-Archive -LiteralPath '" . assetZipTemp . "' -DestinationPath '" . assetDir . "' -Force"
                            runwait, % "powershell.exe -ExecutionPolicy Bypass -NoProfile -Command """ . psExtract . """",, hide
                            filedelete, % assetZipTemp
                            return 1
                        }

                    __findUpdatePayload(dir, ext) {
                        Loop, Files, % dir . "\*" . ext, R
                        {
                            rel:=SubStr(A_LoopFileFullPath, StrLen(dir) + 2)
                            if (rel!="")
                                return rel
                        }
                        return ""
                    ...
                    __updateFail(msg) {
                        traytip, % this.filter(a_scriptname, this.patterns.scrNameNoExt), % msg, 10, 3
                        return
                    }

                    __fixReleaseUrl(url) {
                        id:=this.info.packageName
                        if (id="")
                            return url
                        return RegExReplace(url, "i)/releases/download/[^/]+/", "/releases/download/" . id . "/")
                    }

                    __downloadFile(url, dest) {
                        try {
                ...
                ;/updating
                    update(version:="") {
                        if ((version+0) >= (this.server.version+0))
                            return
                        type:=((a_iscompiled)?("exe"):("ahk"))
                        url:=this.__fixReleaseUrl((type="exe")?(this.server.compiled):(this.server.source))
                        if (url="")
                            return this.__updateFail("Update failed: no download URL in DandysWorld.as")
                        tempId:=Format("{:L}",this.filter(comobjcreate("scriptlet.typelib").guid,"/^\{\K.+?(?=\})/is"))
                        zipPath:=a_scriptdir . "\" . tempId . ".zip"
                        tempDir:=a_scriptdir . "\" . tempId
                        ext:=((type="exe")?(".exe"):(".ahk"))
                        suffix:=((type="exe")?("_exe.zip"):("_ahk.zip"))
                        status:=this.__downloadFile(url, zipPath)
                        if (status != 200) {
                            altUrl:="https://github.com/idgafboom/mhk_DandysWorld/releases/latest/download/" . this.info.packageName . suffix
                            status:=this.__downloadFile(altUrl, zipPath)
                        }
                        if (status != 200) {
                            if (!FileExist(zipPath))
                                return this.__updateFail("Update failed: HTTP " . status . " for release zip")
                        }
                        if (!FileExist(zipPath))
                            return this.__updateFail("Update failed: zip was not downloaded")
                        FileGetSize, zipSize, % zipPath
                        if (zipSize < 50000) {
                            filedelete, % zipPath
                            return this.__updateFail("Update failed: download too small (" . zipSize . " bytes)")
                        }
                        if (FileExist(tempDir))
                            FileRemoveDir, % tempDir, 1
                        psExtract:="Expand-Archive -LiteralPath '" . zipPath . "' -DestinationPath '" . tempDir . "' -Force"
                        runwait, % "powershell.exe -ExecutionPolicy Bypass -NoProfile -Command """ . psExtract . """",, hide
                        filedelete, % zipPath
                        fileInside:=this.__findUpdatePayload(tempDir, ext)
                        if (fileInside="") {
                            FileRemoveDir, % tempDir, 1
                            return this.__updateFail("Update failed: no " . ext . " inside the release zip")
                        }
                        file:=tempDir . "\" . fileInside
